Shader "Lpk/LightModel/ToonLightBase"
{
    Properties
    {
        ...
        // [Space]
        // _RegionColors      ("Region Colors", Color)               = (1,1,1,1)
        // _RegionCoords      ("Region Coordinates", Vector)        = (0.0,0.0,1.0,1.0)
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            Vector _RegionCoords[4]; // 4 alanýn UV koordinatlarý
            float4 _RegionColors[4]; // 4 alanýn renk deðerleri
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                // Renk deðiþtirme koþullarý
                for (int i = 0; i < 4; i++)
                {
                    float4 region = _RegionCoords[i];
                    if (uv.x > region.x && uv.x < region.z && uv.y > region.y && uv.y < region.w)
                    {
                        baseMap.rgb = _RegionColors[i].rgb;
                    }
                }
